...
public class RepositoryConfig implements IRepositoryConfig {
    ...
    public static RepositoryConfig parseFromFile(final File absConfigPath)
        ...
        if (!absConfigPath.exists()) {
            throw new RepositoryConfigurationException("Repository config \""
        ...
        try {
            ...
            BackendServerConfig config = Lookup.getDefault().lookup(BackendServerConfig.class);

            if (props.getSections().contains("mysql") && !config.getNoDatabase()) {
                usesMysql = true;
            ...
            } else if (props.getSections().contains("postgresql") && !config.getNoDatabase()) {
                usesMysql = true;
