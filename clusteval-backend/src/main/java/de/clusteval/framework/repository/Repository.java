...
public class Repository implements IRepository {
    ...
    public Repository(final String basePath, final IRepository parent, final RepositoryConfig overrideConfig)
        ...
        this.parent = parent;
        this.missingRLibraries = new ConcurrentHashMap<>();
     ...
     *
     * @param absFolderPath The absolute path of the folder to ensure.
    ...
    @Override
    public <T extends IRepositoryObject> boolean isClassRegistered(final Class<T> c) {
        return this.isClassRegistered(c, c.getSimpleName());
    }

    @Override
    public <T extends IRepositoryObject> boolean isClassRegistered(final String classFullName) {
        return DynamicRepositoryEntity.isClassAvailable(classFullName);
    }

    @Override
    public <T extends IRepositoryObject> boolean isClassRegistered(final Class<T> base, final String classSimpleName) {
    ...
    @Override
    public <T extends IRepositoryObject> boolean registerClass(final Class<T> c) {
    ...
    public <T extends IRepositoryObject, S extends T> boolean registerClass(final Class<T> base, final Class<S> c) {
        if (!this.dynamicRepositoryEntities.containsKey(base) && base.getSuperclass() != null
                && RepositoryObject.class.isAssignableFrom(base.getSuperclass())) {
            return this.registerClass((Class<IRepositoryObject>) base.getSuperclass(), c);
        }
        return this.dynamicRepositoryEntities.get(base).registerClass(c);
    }

    @Override
    public <T extends IRepositoryObject> boolean unregisterClass(final Class<T> c) {
    ...
    public <T extends IRepositoryObject, S extends T> boolean unregisterClass(final Class<T> base, final Class<S> c) {
        if (!this.dynamicRepositoryEntities.containsKey(base) && base.getSuperclass() != null
                && RepositoryObject.class.isAssignableFrom(base.getSuperclass())) {
            return this.unregisterClass((Class<IRepositoryObject>) base.getSuperclass(), c);
        }
